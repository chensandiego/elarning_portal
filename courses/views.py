...
def course_list(request):
    #avoid n+1 problems
    #use prefetct_related or select_related
        ...
        with transaction.atomic():
            ...
            for key,value in request.POST.items():
                ...
                if answer_id not in question.answer_set.values_list('id',flat=True):
                    raise SuspiciousOperation('answer is not valid for this q')
...
def calculate_score(user,section):
    ...
    correct_answers = UserAnswer.objects.filter(user=user,
           question__section=section,answer__correct=True)
