...
def student_detail(request):
    #check if a user is log in, if not give an error
    if not request.user.is_authenticated():
        raise PermissionDenied
    student = request.user
    return render(request,'students/student_detail.html',{'student':student,'scores':get_all_scores_for_user(student),})
